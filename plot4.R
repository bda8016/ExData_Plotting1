...
#---------------------------------------- ACTIVE PNG GRAPHICS DEVICE AND SET GLOBAL PARAMETERS ---------------------------------------#
...
#Create the basic plot without labeling on the x-axis.  type = "l" specifies a line with no points:
  plot(power$Global_active_power, type = "l", ylab = "Global Active Power", xlab = "", xaxt = "n")
...
#Create the basic plot without labeling on the x-axis.  type = "l" specifies a line with no points:
  plot(power$Voltage/1000, type = "l", xlab = "datetime", ylab = "Voltage", xaxt = "n")
            
#Create x-axis tick marks and customize the x axis:
  tick1 <- 1
    tick2 <- nrow(power)/2
      tick3 <- nrow(power)
        ticks <- c(tick1, tick2, tick3)
...
#---------------------------------------- BOTTOM LEFT PLOT ---------------------------------------#
  ...
  #Add Sub_metering_2 and Sub_metering_3:
   ...
   #Legend
    legend("topright", legend = c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), col = c("black", "orange", "blue"), border = "", lwd = 1)
...
#Create the basic plot without labeling on the x-axis.  type = "l" specifies a line with no points
  plot(power$Global_reactive_power, type = "l", xlab = "datetime", ylab = "Global_reactive_power", xaxt = "n")
